...
import { DeployResult, Receipt } from 'hardhat-deploy/dist/types';
...
export const shouldVerifyFactoryContract = async (factoryDeploy: Receipt): Promise<boolean> => {
  if (process.env.FORK || process.env.TEST) return false;
  const txReceipt = await ethers.provider.getTransaction(factoryDeploy.transactionHash);
  await txReceipt.wait(10);
  return true;
};
